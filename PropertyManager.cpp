...
Monopoly::PropertyManager::PropertyManager(const Monopoly::Board& board) {
  for (const auto& space : board.getSpaces()) {
    if ((*space).getSpaceType() == SpaceType::propertySpace) {
      ...
      // then we can do dynamic casts.
      Property& cRef = dynamic_cast<Property&>(*space);

      if (propertySets.count(cRef.getSetId()) == 0) {
        propertySets[cRef.getSetId()] = PropertySet(cRef.getSetId());
      } else {
        propertySets.at(cRef.getSetId()).incNumPropertiesInSet();
      }
    }
  }
}
